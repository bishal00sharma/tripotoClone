...
import styles from "./Middle6.module.css";
...
export default function Middle6() {
  ...
  useEffect(()=>{
    ...
    function handleCheck(){
      {!isAuth&&alert("Login first")}
      {!isAuth&&window.scrollTo({
        top: 0,
        behavior: "smooth",
      });
    }}
            ...
            {data.map((item)=>(
                <div style={{height:"50%",width:"80%",margin:"auto"}} key={item.id}>
                  ...
                  <div style={{display:"flex",marginTop:"5px",justifyContent:"space-between"}}>
                    ...
                    <div><div style={{color:"white",backgroundColor:"rgb(89, 181, 189)",borderRadius:"10px",padding:"3px 6px"}}>{item.days}</div></div>
                  </div>
                  ...
                  <Button onClick={handleCheck}>{isAuth?<Link  to={`/travelpage/${item.id}`}>Book Now</Link>:<Button>Book Now</Button>}</Button>
                  </div>
                  <hr />
